...
#   TAG=final bash scripts/run_stage2_matrix.sh   # tags every logged row with `step`
set -e
cd "$(dirname "$0")/.."

# Reduce CUDA fragmentation so the topk=900 pool (up to 900 nodes/graph) fits 8GB.
...
for s in $SEEDS; do
  ...
  [ -f "$ckpt" ] && python3 src/gnn/eval_b0.py --ckpt "$ckpt" --seed "$s" --tag "$TAG" \
    || echo "skip B0 seed $s (no ckpt $ckpt)"
done

echo "===== Cand+NMS reference (same candidate pool, no GNN) ====="
python3 src/gnn/eval_pool.py --tag "$TAG"
...
for v in "${VARIANTS[@]}"; do
  ...
  for s in $SEEDS; do
    ...
    python3 src/gnn/train.py --block_a "$A" --block_b "$B" --seed "$s" --tag "$TAG" \
      || echo "FAILED block_a=$A block_b=$B seed=$s (continuing)"
  done
done
